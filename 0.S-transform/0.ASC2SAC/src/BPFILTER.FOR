      subroutine intgr(xc,dt,np)
c
c   integration in time domain
c    xc      :  time trace
c    dt      :  sampling interval (second)
c    np      :  number of data points
      real xc(32768)
      x1=0.5*xc(1)*dt
      do i=1,np-1
        add=0.5*(xc(i+1)+xc(i))*dt
        if(i.eq.1) tmp1=add+x1
        if(i.eq.2) tmp2=add+tmp1
        if(i.ge.3) then
          xc(i-1)=tmp1
          tmp1=tmp2
          tmp2=add+tmp2
      ...
      subroutine HighPassFilter(data1,NP,DT,lowcorner,highcorner)
c Apply a simple band pass filter to data
      real*4 data1(*)
      real*4 lowcorner,highcorner
      call IIRFILT( DATA1, NP, 'BUTTER  ',4,
     +            'HP      ', lowcorner, highcorner, DT, 1 )
      return
      end
