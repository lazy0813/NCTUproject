...
C Curwid is set in routine setlw
   ...
   10 continue
      ...
      if(closer)then
c       cmdstr='Cs'  !Can't use this since we've broken things up into smaller stroke packets
        call linet(xarr(1),yarr(1))
        cmdstr='S'
      else
        cmdstr='S'
      endif
...
c     call date(dat)
      ...
      cmdstr=
     +'%Library Creator: Kevin E. Kohler <kevin@nova.edu>'
      call filler
...
c  Save the length of the substring for comparison with superstring
      call filler
      cmdstr='/Sbstring Strlen def'
...
c  Save the length of the superstring for comparison with substring
      call filler
      cmdstr='/Spstring Strlen def'
...
C  Compute next character position
c  Use the longest of substring and superstring when position next character
      cmdstr='Sbstring Spstring gt {/Strlen Sbstring def}'
     . //'{/Strlen Spstring def} ifelse'
      call filler
...
c  Set character size
      ...
      cmdstr='/sbstring 0 def /spstring 0 def'
      call filler
...
c  Add to sbstring
        cmdstr='/sbstring sbstring Strlen add def'
        call filler
...
C  Compute next character position
   ...
   10 continue
      ...
      cmdstr='/sbposx Xpres def /sbposy Ypres def'
      call filler
...
c  Add to spstring
        cmdstr='/spstring spstring Strlen add def'
        call filler
...
C  Compute next character position
   ...
   15 continue
      cmdstr='/spposx Xpres def /spposy Ypres def'
      call filler
 
c  Choose which position to use
      cmdstr='/sbx sbposx Xposv sub def /sby sbposy Yposv sub def'
      call filler
      cmdstr='/spx spposx Xposv sub def /spy spposy Yposv sub def'
      call filler
      cmdstr='/sbdist sbx sbx mul sby sby mul add def'
      call filler
      cmdstr='/spdist spx spx mul spy spy mul add def'
      call filler
 
c  Use the longest of substring and superstring when position next character
      cmdstr='sbdist spdist gt {/Xpres sbposx def /Ypres sbposy def}'
     . //'{/Xpres spposx def /Ypres spposy def} ifelse'
      call filler
...
c  Reset angle
      ...
      subroutine newdate(dat,tim)
      integer date_time(8)
      character*10 dummyarg
      character*(*)dat,tim
      character*36 monnam
      data monnam/'JANFEBMARAPRMAYJUNJULAUGSEPOCTNOVDEC'/

      call date_and_time(dummyarg,dummyarg,dummyarg,date_time)

c  Convert new date/time to the old way

      write(dat(1:2),'(i2.2)')date_time(3)
      mind=(date_time(2)-1)*3+1
      dat(4:6)=monnam(mind:mind+2)
      iyronly=date_time(1)-(date_time(1)/100)*100
      write(dat(8:9),'(i2.2)')iyronly
      dat(3:3)='-'
      dat(7:7)='-'

      write(tim(1:2),'(i2.2)')date_time(5)
      write(tim(4:5),'(i2.2)')date_time(6)
      write(tim(7:8),'(i2.2)')date_time(7)
      tim(3:3)=':'
      tim(6:6)=':'
      return
      end